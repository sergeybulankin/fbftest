$('document').ready(function() {
	...
	$('.sendsubmit').click(function(){
		...
		$.ajax({
		  type: "POST",
		  url: '../tchf-test/avtorization.php',
		  ...
		  success: function(data){
			  ...
			  if (data.result == 'error') {
			  	...
			  	//пользователь найден, перекидываем на страницу с вопросами
				if(data.payment != 0) {
					window.location.reload('../tchf-test/protected.php');
				}
				{
					$('.my_error').css({'display':'block','position': 'absolute', 'top': '-85px'}).html('Уважаемый участник! <br><b>Факт оплаты Вами оргвзноса не установлен. </b><br>Пожалуйста, зарегистрируйтесь повторно, оплатив онлайн, либо отправьте скан-копию чека на почту tchftest.strbsu@yandex.ru.');
				}
			  }
		  },
		  dataType: 'html',
	...
	//на странице отображения вопросов
	if ($('.navigation').length !== 0) {
	  ...
	  $('li[data-num-question]').click(function() {
		  ...
		  //ОТОБРАЖАЕМ ВОПРОС
				$.ajax({
				  type: "POST",
				  url: '../tchf-test/get_question.php',
				  data: { number:number },
				  success: function(data){
	 ...
	 $('#yes').click(function(){
		 console.log('выход');
		 window.location.href="../tchf-test/exit.php";
	 });
