<?php
    ...
    include "../Open_db.php";
     ...
     if (count($res) !== 0) {   //пользователь найден
		...
		if(($payment+$payment_online) != 0) {
		 ...
		 if ($class <= 4) $_SESSION['category'] = 1;

		 if (($class >= 5) & ($class <= 8)) $_SESSION['category'] = 2;

		 if (($class >= 9) & ($class<=11)) $_SESSION['category'] = 3;

		 if ($res[0]['cat'] == 2) $_SESSION['category'] = 4;
