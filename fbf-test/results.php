<?php
	...
	echo '</style>';
					$students=get_raw("SELECT * FROM `students_2019_1`");
					...
					for ($i=0;$i<count($students);$i++)
					{
						...
						$query="SELECT `questions`.`id_questions`,`questions`.`number_true`, `answers_2019_1`.`answer` ";
						$query.=" FROM `questions` INNER JOIN `answers_2019_1` ON `questions`.id_questions = `answers_2019_1`.id_questions ";
						$query.=" WHERE `answers_2019_1`.`id_students`=".$students[$i]['id_students']." AND `answers_2019_1`.`answer`=`questions`.`number_true`";
						$count_true_answers_student=count(get_raw($query));//кол-во верных ответов
						 ...
						 if ($students[$i]['cat']==2) {
							$category=4;
							$this_answer=get_raw("SELECT * FROM `answers_2019_1` WHERE `id_students`='".$students[$i]['id_students']."'");
							
							if (count($this_answer)!==0)
								{
								$new_cat=get_raw("SELECT * FROM `questions` WHERE `id_questions` IN (SELECT `id_questions` FROM `answers_2019_1` WHERE `id_students`='".$students[$i]['id_students']."')");
						...
						$payment=get_raw("SELECT * FROM `payment_2019_1` WHERE `id_students`=".$students[$i]['id_students']);
						...
						$parents=get_raw("SELECT * FROM `parents_2019_1` WHERE `id_srudents`=".$students[$i]['id_students']);
						echo '<td>';
